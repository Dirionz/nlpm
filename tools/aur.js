...
exports.install = (pkg, callback) => {
    ...
    cmd.start("perl", [__dirname+'/trizen', '-Sy', pkg, '--nocolors'], function(err, data) {
        if (err) { 
...
exports.restore = (pkgs, callback) => {
    ...
    //    }
    //});
    cmd.start("perl", [__dirname+"/trizen", '-Sy', '--noconfirm', '--nocolors'].concat(pkgs.map(x => x.name)), function(err, data) {
        if (err) { 
            callback({'msg': 'Error when trying to install: ' + pkg + " -> " + err });
        } else {
            callback();
...
exports.update = (callback) => {
    ...
    //    }
    //});
    cmd.start("perl", [__dirname+"/trizen", '-Syu', '--noconfirm', '--nocolors'], function(err, data) {
