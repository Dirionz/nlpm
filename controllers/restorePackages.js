...
// base-devel git diffutils perl perl-libwww perl-term-ui perl-json perl-data-dump perl-lwp-protocol-https
...
function managerTask(managers, callback) {
    ...
    switch(manager.name) { // TODO: Handle error with put in logfile?
        case 'npm':
            npm.restore(manager.packages, function(err) { // TOOD: Change to restore funcitons!
                notifyUser(err, manager.name);
                callback(err, managers)
            });
            break;
        case 'pacman':
            pacman.restore(manager.packages, function(err) { 
                notifyUser(err, manager.name);
                callback(err, managers)
            })
            break;
        case 'apt':
            apt.restore(manager.packages, function(err) { 
                notifyUser(err, manager.name);
                callback(err, managers)
            })
            break;
        case 'aur':
            aur.restore(manager.packages, function(err) {
                notifyUser(err, manager.name);
                callback(err, managers);
            });
            break;
